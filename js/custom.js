...
function execDrop(evt) {
  ...
  if (mimecheck.startsWith('audio')) {
    ...
    var audioTag = document.getElementById('usermusic');
...
　
    ...
    for(var i=0, len=spectrums.length; i<len; i++){
      ...
      if (i%3 === 0){
        canvasContext.fillRect(i*10, 80, 5, spectrums[i]/6);
        canvasContext.fillRect(i*10, 70, 5, 10);
        canvasContext.fillRect(i*10, 70, 5, -(spectrums[i]/6));
      } else if (i%5 === 0) {
        canvasContext.fillRect(i*10, 80, 5, spectrums[i]/2);
        canvasContext.fillRect(i*10, 70, 5, 10);
        canvasContext.fillRect(i*10, 70, 5, -(spectrums[i]/2));
      } else {
        canvasContext.fillRect(i*10, 80, 5, spectrums[i]/5);
        canvasContext.fillRect(i*10, 70, 5, 10);
        canvasContext.fillRect(i*10, 70, 5, -(spectrums[i]/5));
      }
    }
　
    animationId = requestAnimationFrame(render);
